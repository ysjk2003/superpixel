...
function distanceTransform2D(distanceMap) {
  ...
  // Column transform.
  for (x = 0; x < width; ++x) {
    for (y = 0; y < height; ++y) f[y] = data[y * width + x]
  ...
  for (y = 0; y < height; ++y) {
    ...
    for (x = 0; x < width; ++x) data[y * width + x] = d[x]
  }
  // Sqrt.
  for (x = 0; x < data.length; ++x) data[x] = Math.sqrt(data[x])
}

export function distanceTransform(intensity, options) {
  options = options || {}
  var distanceMap = {
