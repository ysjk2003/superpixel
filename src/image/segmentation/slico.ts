...
export default class SLICO extends BaseSegmentation {
  private width: number
  private height: number
  private method: string
  private K: number
  private perturb: undefined | boolean
  private maxIterations: number
  private step: number
  private enforceConnectivity: boolean
  private lvec!: Float64Array
  private avec!: Float64Array
  private bvec!: Float64Array
  private estep: number = 0
  private kSeedsL!: Float64Array | number[]
  private kSeedsA!: Float64Array | number[]
  private kSeedsB!: Float64Array | number[]
  private kSeedsX!: Float64Array | number[]
  private kSeedsY!: Float64Array | number[]
  private edges!: Float64Array | Int32Array
  ...
  _compute() {
    const labels = this.method === "FixedK" ? this.performSLICOForGivenK() : this.performSLICOForGivenStepSize()
  ...
  getLABXYSeedsForGivenStepSize(step: number, perturb?: boolean) {
    ...
    for (let y = 0; y < ystrips; ++y) {
      ...
      for (let x = 0; x < xstrips; ++x) {
        ...
        const i = Math.floor((y * step + yoff + ye) * this.width + (x * step + xoff + xe))
  ...
  performSuperpixelSegmentationVariableSandM(kLabels: Float64Array | Int32Array, step: number, maxIterations: number) {
    const size = Math.floor(this.width * this.height),
        ...
        for (y = y1; y < y2; ++y) {
          for (x = x1; x < x2; ++x) {
            ...
            const l = this.lvec[i],
              ...
              b = this.bvec[i]
            distlab[i] =
              Math.pow(l - this.kSeedsL[n], 2) + Math.pow(a - this.kSeedsA[n], 2) + Math.pow(b - this.kSeedsB[n], 2)
  ...
  enforceLabelConnectivity(labels: Float64Array | Int32Array, nlabels: Float64Array | Int32Array, K: number) {
    const dx4 = [-1, 0, 1, 0],
      dy4 = [0, -1, 0, 1],
...
function encodeLabels(labels: Int32Array | Float64Array, data: Uint8ClampedArray) {
  for (let i = 0; i < labels.length; ++i) {
    const label = labels[i]
